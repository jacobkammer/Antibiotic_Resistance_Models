...
def dual_reservoir_model(y, t, params, van_func, lzd_func, immune_model):
    ...
    #   E_L(t) is a bacteriostatic effect rate (h⁻¹) subtracted from the growth rate.
    #   Emax_l = rho_S = 0.30 h⁻¹: complete growth arrest of sensitive strain at saturation.
    #   E_L ∈ [0, Emax_l]; (rho_S - E_L) ≥ 0 — drug cannot directly kill via this term.
    h_L = 1.0
    E_L = (params['Emax_l'] * L**h_L) / (params['EC50_L']**h_L + L**h_L)
    # E_L ∈ [0, Emax_l = rho_S]:  0 = no effect,  rho_S = complete growth arrest
    ...
    # Blood — Resistant (R_b): vancomycin-resistant only; linezolid fully active.
    dR_b = (
    ...
    E_L_res = (Emax_l_res * (lzd_res_fraction * L)**h_L) / \
              (params['EC50_L']**h_L + (lzd_res_fraction * L)**h_L)
...
if __name__ == "__main__":
    ...
    # =========================================================================
    rho_S = 0.30    # h⁻¹  S. aureus in vivo apparent growth rate
                    # Note: in vitro value (Campion 2005) is 0.63 h⁻¹, but in vivo
                    # ODE models use lower apparent rates (~0.2–0.35 h⁻¹) reflecting
                    # nutrient limitation, suboptimal temperature, and in vivo stress.
                    # At 0.30: net blood growth = 0.30-0.12 = 0.18 h⁻¹ → peak at ~2-3 days ✓
                    # (Mouton et al. 2018; Nielsen et al. 2011 clinical PKPD analyses)
    rho_R = (1 - resistance_factor) * rho_S   # 0.504 h⁻¹ at rf=0.20
    ...
    params = {
        ...
        # ---- Linezolid PD ----
        # Emax_l: max inhibition fraction (1.0 = complete growth arrest)
        # Since linezolid is bacteriostatic, this is dimensionless [0,1]
        'Emax_l':  1.0,    # dimensionless fraction [0,1]
        ...
        # This factor multiplies EC50_V in the reservoir compartment only.
        # Lewis 2001 AAC; Ceri et al. 1999 AAC
    ...
    # =========================================================================
    # CORRECTION: S_b initialized to small positive value (not 0)
    # Without this, with f_r_b = 5e-4 h⁻¹ and S_res = 10, initial seeding
    # is 0.005 CFU/mL/h — very slow. Starting S_b at the inoculum level
    # (e.g., after hematogenous seeding) is more realistic.
    # Blood inoculum: realistic hematogenous seed 5–10 CFU/mL (not 100)
    # 100 CFU/mL caused bacteria to saturate B_max within 12 h of a 24 h pre-treatment window
    y0 = [5, 1, 1e4, 1e3]   # [S_b, R_b, S_res, R_res] in CFU/mL
